...
discard = 750                                                  # Discard first `discard` iterations as burn-in
...
if __name__ == '__main__':
    ...
    forecasts.to_csv(os.path.join(os.path.dirname(__file__), f'../../data/interim/calibration/forecast/{model_name}/hyperparameters-{hyperparameters}/{(end_date+timedelta(weeks=1)).strftime('%Y-%m-%d')}-Cornell_JHU-hierarchSIR.csv'), index=False)